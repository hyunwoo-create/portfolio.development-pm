...
<body class="min-h-screen">
  ...
  <div id="main-content" class="pt-[100px] pb-16 px-8 max-w-[800px] w-full mx-auto bg-[#F9F7F7]">
    ...
    <div class="mb-12">
      ...
      <p class="text-[#8fabc8] text-base font-medium">${r||"게임 기획부터 런칭까지의 메인 프로젝트와 AI/툴링을 활용한 기타 작업물 아카이브입니다."}</p>
    </div>

    <!-- Categories Display (static design) -->
    <div class="flex flex-wrap gap-3 mb-10 pb-6 border-b border-[#DBE2EF]/50">
      ${a.map((t,e)=>`
        <div class="px-5 py-2.5 rounded-full text-sm font-bold border flex items-center gap-2 ${e===0?"bg-[#112D4E] text-white border-[#112D4E] shadow-md":"bg-white text-[#8fabc8] border-[#DBE2EF]"}">
          ${t}
        </div>
      `).join("")}
    </div>
    ...
    <div class="grid grid-cols-2 gap-8 avoid-break">
      ${i.map(t=>{var e;return`
        <a href="#" data-project-id="${t.id}" class="project-card group flex flex-col bg-white rounded-3xl border border-[#DBE2EF] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 relative cursor-pointer no-underline block avoid-break" target="_blank">
          <div class="aspect-[4/3] relative overflow-hidden bg-[#F9F7F7]">
            <img
              src="${t.image||"https://images.unsplash.com/photo-1550745165-9bc0b252726f?q=80&w=2070"}"
              alt="${t.title}"
          ...
          <div class="p-8 flex-1 flex flex-col">
            <div class="text-[#C08D50] font-bold text-xs mb-2 tracking-widest uppercase">
              ${((e=t.details)==null?void 0:e[0])||"서브 카테고리"}
            </div>
  ...
  `;export{s as generatePortfolioHtml};
