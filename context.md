...
### 1. PDF 포트폴리오 추출 기능 최적화
- **불필요한 UI 제거**: PDF로 추출되는 HTML 화면(`html-generator.ts`)에서 카테고리 탭(게임 분석, AI 활용 등) UI 요소를 완전히 삭제하여 깔끔한 리스트만 남도록 정리.
- **1페이지 레이아웃 압축**: 상단 네비게이션 버튼과 타이틀 영역의 과도한 여백(margin, padding)을 대폭 축소. 프로젝트 그리드 카드들이 1페이지를 넘어가더라도 `avoid-break` 속성을 컨테이너에서 제거하여 빈번한 페이지 밀림을 방지함.
