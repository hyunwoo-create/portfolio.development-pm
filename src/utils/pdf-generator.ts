 ...
 */
export const handlePdfExport = (data: ResumeData) => {
  try {
    // 1. PDF 전용 정적 마크업 생성 (Interactive 요소 배제)
    const htmlContent = renderToStaticMarkup(React.createElement(ResumePDF, { data }));
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>${data.name || 'Resume'}_Export</title>
          <meta charset="utf-8">
          ${styles}
          <style>
            /* PDF 전용 인쇄 설정 */
